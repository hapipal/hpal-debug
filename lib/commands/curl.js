...
module.exports = async (server, args, root, ctx) => {
    ...
    const parameters = internals.parameters({
        id,
        method,
        path,
        matchOn,
...
internals.parameters = (info, args, ctx) => {
    ...
    const definition = Object.assign(
        internals.makeDefinition((matchOn[0] === 'id') ? paramsInfo : {}, 'params'),
    ...
    const getUsage = () => {
        ...
        let usage = [
            'hpal run debug:curl <id> [options]',
            'hpal run debug:curl [<method>] <path> [options]'
        ].join('\n       ');

        const actualArgs = args.slice(0, matchOn.length).join(' ');

        if (actualArgs) {
            usage += '\n\n' + ctx.colors.bold(`hpal run debug:curl ${actualArgs} [options]`);
        }

        return Bossy.usage(definition, usage, { colors: ctx.options.colors });
    };

    if (!route) {
        if (matchOn.length === 0) {
            throw new ctx.DisplayError(getUsage() + '\n\n' + ctx.colors.red('No route specified'));
        }
        if (matchOn[0] === 'id') {
            throw new ctx.DisplayError(ctx.colors.yellow(`Route "${id}" not found`));
        }
        throw new ctx.DisplayError(ctx.colors.yellow(`Route "${method} ${path}" not found`));
    }

    const parameters = Bossy.parse(definition, {
        argv: args.slice(matchOn.length)
    });
