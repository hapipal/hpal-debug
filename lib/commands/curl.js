...
module.exports = async (server, args, root, ctx) => {
    ...
    const paramsInfo = (matchOn[0] === 'id') && params && params.isJoi && params.describe().children || {};
...
internals.parameters = (info, args, ctx) => {
    ...
    const { options, colors, DisplayError } = ctx;
    ...
    if (!route) {   // Note that matchOn is [], the other cases have been handled
